...
import { ArrowLeft, CheckCircle2, Film, CalendarDays, Clock } from "lucide-react";
...
const ShowtimeForm = ({ movies, rooms, allShowtimes, onBack, onSuccess }) => {
  ...
  const displayedShowtimes = useMemo(() => {
    ...
    if (viewMode === "MOVIE") {
      ...
      filtered = allShowtimes.filter((s) => s.movie.id === Number(formData.movieId));
    } else {
      if (!formData.startTime) return [];
      const selectedDate = formData.startTime.split("T")[0]; 
      
      filtered = allShowtimes.filter((s) => s.startTime.startsWith(selectedDate));
    }
  ...
  const calculateEndTime = (startStr, duration) => {
    ...
    const endDate = new Date(new Date(startStr).getTime() + (duration + 15) * 60000);
  ...
  const handleSubmit = async (e) => {
    ...
    await execute(
      () =>
        axiosClient.post(
          formData.isAutoGenerate ? "/admin/showtimes/auto-generate" : "/admin/showtimes",
          {
      ...
      {        
        onSuccess: (res) => {
          ...
          if (formData.isAutoGenerate) {
            ...
            if (createdCount === 0) {
              toast.error("⚠️ Không thể sinh lịch chiếu nào! (Có thể do trùng giờ hoặc quá giờ đóng cửa)");
              return; 
            }
            
            toast.success(`Đã tự động sinh thành công ${createdCount} suất chiếu!`);
          } 
          else {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-white font-body">
      ...
      <div className="lg:col-span-1 bg-neutral-800 p-6 rounded-xl border border-neutral-700 shadow-xl h-fit">
        <button onClick={onBack} className="text-neutral-400 hover:text-white mb-6 flex items-center gap-2 text-sm font-bold">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-neutral-400 text-sm font-bold mb-2">Chọn Phim</label>
            <select
          ...
          <div>
            <label className="block text-neutral-400 text-sm font-bold mb-2">Chọn Phòng</label>
            <select
              className="w-full p-3 bg-neutral-900 border border-neutral-600 rounded-lg text-white focus:border-red-500 outline-none transition-colors"
          ...
          <div className="grid grid-cols-1 gap-4">
            <div>
              <label className="block text-neutral-400 text-sm font-bold mb-2">Bắt đầu</label>
              <input
                type="datetime-local"
                min={minDateTime}
            ...
            <div>
              <label className="block text-neutral-400 text-sm font-bold mb-2">Giá vé</label>
              <input
                type="number"
                step="5000" min="45000" max="200000"
          ...
          {calculatedEndTime && (
            <div className="bg-neutral-900/50 p-3 rounded border border-dashed border-neutral-600 text-xs flex justify-between">
              <span className="text-neutral-400">Kết thúc: <b className="text-red-500 ml-1">{calculatedEndTime}</b></span>
              <span className="text-neutral-500">({selectedMovieDuration}p + 15p dọn)</span>
            </div>
          )}
          ...
          <div
            ...
            className={`p-4 rounded-lg border cursor-pointer flex items-center gap-3 transition-colors ${
              formData.isAutoGenerate ? "bg-red-900/20 border-red-500" : "bg-neutral-900 border-neutral-700"
            }`}
          >
            <div className={`w-5 h-5 rounded flex items-center justify-center border ${
              formData.isAutoGenerate ? "bg-red-600 border-red-600 text-white" : "border-neutral-500"
            }`}>
              {formData.isAutoGenerate && <CheckCircle2 size={14} />}
            </div>
            <div>
              <h4 className="font-bold text-sm text-white">Tự động sinh lịch</h4>
              <p className="text-[10px] text-neutral-500">Xếp lịch liên tục đến 23:00</p>
            </div>
          </div>
      ...
      <div className="lg:col-span-2 bg-neutral-800 rounded-xl border border-neutral-700 overflow-hidden flex flex-col h-[600px]">
        ...
        <div className="bg-neutral-900 border-b border-neutral-700">
            <div className="flex">
                ...
                >
                    <Film size={16} /> Theo Phim {viewMode === "MOVIE" && `(${displayedShowtimes.length})`}
                </button>
                <button 
                ...
                >
                    <CalendarDays size={16} /> Theo Ngày {viewMode === "DATE" && `(${displayedShowtimes.length})`}
                </button>
            </div>
            
            <div className="px-4 py-2 text-xs text-neutral-500 bg-neutral-900 text-center border-b border-neutral-800">
                {viewMode === "MOVIE" 
                    ? (formData.movieId ? "Đang hiện các suất của phim này (tất cả ngày)" : "Hãy chọn phim để xem lịch")
                    : (formData.startTime ? `Lịch chiếu ngày ${formData.startTime.split('T')[0]}` : "Hãy chọn ngày bắt đầu để xem lịch rạp")}
            </div>
        </div>
                  ...
                  {shows.map((show) => (
                    <div
                      ...
                      className={`p-3 rounded border text-xs relative group transition-all ${
                         viewMode === "DATE" ? "bg-neutral-700/50 border-neutral-600" : "bg-neutral-900 border-neutral-700"
                      }`}
                    >
                      ...
                      <div className="text-neutral-400 flex justify-between items-center">
                         ...
                         {viewMode === "DATE" && (
                            <span className="text-[10px] text-yellow-500 font-bold truncate max-w-[80px] ml-2" title={show.movie.title}>
                                {show.movie.title}
                            </span>
                         )}
                      </div>

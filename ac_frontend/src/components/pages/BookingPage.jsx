    ...
    Swal.fire({
        ...
        html: `
            <div style="text-align: left; font-size: 0.9em;">
                ...
                <p>Tổng tiền: <b style="color: #EAB308; font-size: 1.2em;">${formatCurrency(totalPrice)}</b></p>
            </div>
        `,
        icon: 'question',
        background: '#171717',
    ...
    <div className="min-h-screen bg-neutral-900 text-white flex flex-col">
        ...
        <div className="min-w-[600px] flex flex-col items-center gap-3 mx-auto">
          {Object.entries(seatsByRow).map(([rowLabel, rowSeats]) => (
            <div key={rowLabel} className="flex items-center gap-4">
              <span className="w-6 text-center text-neutral-500 font-bold">
                {rowLabel}
              </span>
              ...
              <div className="flex gap-2 sm:gap-3">
                {rowSeats.map((seat) => {
                  ...
                  let seatStyle =
                    "bg-neutral-800 border-neutral-600 text-neutral-400 hover:border-yellow-500 hover:text-yellow-500 hover:shadow-[0_0_10px_rgba(234,179,8,0.4)]";

                  if (seat.isBooked) {
                    seatStyle =
                      "bg-neutral-800/50 border-neutral-800 text-neutral-700 cursor-not-allowed";
                  } else if (isSelected) {
                    seatStyle =
                      "bg-yellow-500 border-yellow-500 text-neutral-900 font-bold shadow-[0_0_15px_rgba(234,179,8,0.6)] transform scale-110";
                  } else if (isVip) {
                    seatStyle =
                      "border-red-500 text-red-500 hover:bg-red-500/10";
                  } else if (isCouple) {
                    seatStyle =
                      "border-pink-500 text-pink-500 w-20 sm:w-24 rounded-lg hover:bg-pink-500/10";
                  }
      ...
      <div className="flex justify-center gap-4 sm:gap-8 text-xs sm:text-sm text-neutral-400 mb-24 px-4 flex-wrap">
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 bg-neutral-800 border border-neutral-600 rounded"></div>{" "}
          Thường
        </div>
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 border border-red-500 rounded"></div> VIP
        </div>
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 border border-pink-500 rounded w-8"></div>{" "}
          Couple
        </div>
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 bg-yellow-500 rounded"></div> Đang chọn
        </div>
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 bg-neutral-800 border-neutral-800 text-neutral-700 flex justify-center items-center">
            X
          </div>{" "}
          Đã đặt
        </div>
